...
import { Plus, Check, Coffee, Loader2 } from "lucide-react";
...
export default function ProductCard({ product }: { product: any }) {
  ...
  const handleAddToCart = () => {
    ...
    toast.success(`${product.name} ditambahkan!`, {
      style: { background: "#4B2E1C", color: "#FDF6EE", borderRadius: "12px" },
    });
    setIsAdded(true);
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-[#8B5E3C]/10 flex flex-col group h-full">
            ...
            {product.notes && product.notes.length > 0 && (
              <div className="bg-[#FDF6EE] p-3 rounded-xl border border-[#8B5E3C]/10">
                ...
                <p className="text-xs font-teks text-[#4B2E1C] italic font-medium leading-relaxed">
                  &ldquo;{product.notes.join(" • ")}&rdquo;
                </p>
              </div>
            )}
          </div>
